                ...
                if (currentVideoSource != null)
                {
                    if (captionEnabled.Value)
                    {
                        Task.Run(async () =>
                        {
                            ...
                            var tracksWithoutAutoGenItems = trackManifest.Tracks.Where(track => track.IsAutoGenerated == false).ToList();

                            var trackInfo = tracksWithoutAutoGenItems.Where(track => track.Language.Name == lang.NewValue.Name).First();
                ...
                if (currentVideoSource != null)
                {
                    if (captionEnabled.Value)
                    {
                        Task.Run(async () =>
                        {
                            ...
                            var trackInfo = tracksWithoutAutoGenItems.Where(track => track.Language.Code.Contains(preferedLang)).First();
                        ...
                        if (!File.Exists(app.Host.CacheStorage.GetStorageForDirectory("videos").GetFullPath($"{this.videoId}") + @"/audio.ogg") || !File.Exists(app.Host.CacheStorage.GetStorageForDirectory("videos").GetFullPath($"{this.videoId}") + @"/video.webm"))
                        {
                            ...
                            try
                            {
                                if (captionEnabled.Value)
                                {
                                    ...
                                    if (tracksWithoutAutoGenItems.Count == 0)
                                    {
